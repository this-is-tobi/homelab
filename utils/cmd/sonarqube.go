...
import (
	...
	"github.com/this-is-tobi/homelab/utils/internal/sonarqube"
)

var sonarqubeCmd = &cobra.Command{
	Use:   "sonarqube",
	Short: "SonarQube post-install configuration",
...
func runSonarqubeConfig(cmd *cobra.Command, args []string) error {
	// Try loading from K8s secrets
	if err := loadSonarqubeFromK8s(); err != nil {
	...
	if err := sonarqube.Configure(cfg); err != nil {
...
func loadSonarqubeFromK8s() error {
